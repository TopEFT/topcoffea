...
def print_table(vals_dict,key_order,subkey_order,caption_text,print_errs,columns,hz_line_lst,tabsize,roundat):
        ...
        for i,key in enumerate(key_order):
            if key not in vals_dict.keys():
                ...
                for subkey in subkey_order:
                    ...
                    if val is not None: val = round(val,roundat)
                    if err is not None: err = round(err,roundat)
...
def print_latex_yield_table(vals_dict,key_order=None,subkey_order=None,tag="",print_begin_info=False,print_end_info=False,print_errs=False,column_variable="subkeys",hz_line_lst=[None],size="normalsize",roundat=2):
    ...
    print_table(vals_dict,key_order,subkey_order,tag,print_errs,columns=column_variable,hz_line_lst=hz_line_lst,tabsize=size,roundat=roundat)
