...
def make_json(sample_dir,sample_name,prefix,sample_yr,xsec_name,hist_axis_name,xsec_value=None,era=None,on_das=False,include_lhe_wgts_arr=False,skip_file_name=[],just_write=False,postfix='',post_mortem=None):
    ...
    args = [
        ...
        das_flag,
        include_lhe_wgts_arr_flag,
        "--sampleName"   , sample_name+postfix,
        "--prefix"       , prefix,
        "--xsec"         , path_to_xsecs,
        "--year"         , sample_yr,
        "--histAxisName" , hist_axis_name,
    ]
    if just_write: args.append("--just_write")
    if post_mortem is not None: args.extend(["--post_mortem", ','.join(post_mortem)])
