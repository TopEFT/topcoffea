...
class SparseHist():
    ...
    def __dask_postcompute__(self):
        def post(vs):
            ...
            return SparseHistResult(self.category_names, {k: h for (k, h) in pairs})
        return post, ()
...
class SparseHistResult():
    def __init__(self, category_names, histograms=None, dense_axis=None):
        ...
        if (not histograms and not dense_axis):
            raise ValueError("Exactly only one of histograms or dense_axis should be specified.")

        self._check_args(category_names)

        self._dense_axis = dense_axis
        if not dense_axis:
            first = next(iter(histograms.values()))
            if len(first.axes) != 1:
                raise ValueError("Dense histograms cannot have more than one axis.")
            self._dense_axis = first.axes[0]
    ...
    def empty_from_axes(self, category_names=None, dense_axis=None, **kwargs):
        ...
        return type(self)(category_names, dense_axis=dense_axis)

    def make_dense(self):
        return hist.Hist(self.dense_axis)
    ...
    def __getitem__(self, key):
        if isinstance(key, dict):
            if len(key) == self._n_categories:
                ...
                axes = chain(self.category_names, [self._dense_axis.name])
            else:
                raise KeyError(key)
    ...
    def integrate(self, axis_name: str, value=None):
        ...
        for k, h, in self._dense_hists.items():
            if value == sum or k[index] == value:
                ...
                new_hists[new_key] += h
        return SparseHistResult(new_categories, new_hists)
    ...
    def group(self, axis_name: str, groups: dict[str, list[str]]):
        ...
        for k, h, in self._dense_hists.items():
            ...
            new_hists[new_key] += h
        return SparseHistResult(self.category_names, new_hists)
